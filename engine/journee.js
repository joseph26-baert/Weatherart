...
export function calculerToutes(oeuvres, villes, donnees, fixes = {}) {
  ...
  for (let j = 0; j < nbJours; j++) {
    for (const p of ['jour']) {
      // 1er passage : inscrire d'abord toutes les œuvres déjà fixées du jour (une ville peut être fixée avant une autre à cause des fuseaux)
      villes.forEach((v, vi) => {
        ...
        if (fixe && byId[fixe.id]) {
          ...
          if (j === nbJours - 1) { const m = bilan(d, j, 6, 22, 'jour'); const profil = profilMeteo(m, dateISO, v.lat); const o = byId[fixe.id]; resultats.set(v.nom, { dateISO, jour: { oeuvre: o, profil, meteo: m, echo: fixe.echo || phraseEcho(o, profil, v.nom), niveau: 1, fixe: true } }); }
        }
      });
      // 2e passage : calculer les villes restantes
      villes.forEach((v, vi) => {
        const d = donnees[vi]; const dateISO = d.daily.time[j];
        if (fixes[`${v.nom.toUpperCase()}+${dateISO}`] && byId[fixes[`${v.nom.toUpperCase()}+${dateISO}`].id]) return;
